...
70% of your work isn't the work. Notes. To-do lists. "Can you send that?" Reminders nobody reads. Follow-up meetings about the last meeting. Ambient flips that — so you stay in the zone while the research, the action items, and the loose threads get handled in the background.

## Core Features

### It Listens
Every conversation, perfectly remembered — in any language, without you ever reaching for a notepad. Real-time system audio capture via ScreenCaptureKit with optional microphone input. Works across 40+ languages without configuration.

### It Structures
Ambient catches what matters and surfaces thoughtful, well-researched suggestions, with multiple agents pulling context in parallel to surface the most critical ones. Tasks, key decisions, agreements, and the things everyone will forget by tomorrow — extracted live and classified as agent-doable or human-only.

### It Actions In Parallel
Ambient turns well-researched suggestions into action, dispatching long-running agents to tackle tasks in parallel. Each agent plans, executes with tool access, and reports back — while you keep talking.

## Any conversation. One tool.

Conversations are where plans are born — and too often, where they disappear. Ambient exists to ensure that what matters doesn't get lost.

### Team Meetings — so we don't have to circle back

Ideas are discussed. Decisions get made. Someone says, "Let's circle back." Ambient captures action items, owners, and decisions in real time, then sends structured notes and tasks to your tools before the meeting even ends.

https://github.com/ambientintelligence-sh/ambient/raw/main/docs/media/ambient-meeting.webm

### Trip Planning — the trips will always make it out of the group chat

Flights get mentioned. Hotels are debated. Someone says, "I'll look into it." Ambient captures destinations, dates, and ideas as your group discusses. Then generates a structured plan with flight research, hotel options, and an itinerary pushed to Notion before the conversation even ends.

https://github.com/ambientintelligence-sh/ambient/raw/main/docs/media/ambient-trip-planning.webm

### Research & Brainstorming — where ideas go to grow
...
### Study Sessions — your intelligence backed by Ambient Intelligence

Concepts are explained. Deadlines are mentioned. Assignments are clarified. Ambient captures the discussion and turns it into summaries, flashcards, and key takeaways — like a study partner that remembers everything you covered.

https://github.com/ambientintelligence-sh/ambient/raw/main/docs/media/ambient-study.webm

### Native Codex — built for the terminal-native way of working
...
- **Continual Learning** — Agents remember all past actions and corrections. Never make the same mistake twice.
...
- **Context-Aware Transcription** — Powered by Gemini Flash with custom context injection. Add speaker names, glossary terms, and project-specific vocabulary so Ambient transcribes names and terminology correctly.
- **Natively Multilingual** — Auto-detects language from character sets (CJK, Arabic, Cyrillic, etc.) and handles code-switching within a single conversation.

## Privacy

- Ambient only listens when you activate it. No continuous streaming.
- Audio is processed locally on your device. No recording files are stored.
- Transcripts and tasks are only saved if you choose to keep them. Sessions can be deleted at any time.
- Every action that touches an external tool (Notion, task managers, etc.) requires your approval.
