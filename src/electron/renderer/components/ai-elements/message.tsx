...
export const MessageBranchContent = ({
  ...
  // Sync branch count — only fires when the number of children actually changes
  useEffect(() => {
    if (branches.length !== childCount) {
      setBranches(childrenArray);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- deliberately keyed on childCount, not childrenArray reference
  }, [childCount]);
...
export function CitedMessageResponse({
  ...
  const { body, sources } = parseCitations(content);
