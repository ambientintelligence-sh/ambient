...
export const REALTIME_INSTRUCTIONS = [
  ...
  'Workers have a show_widget tool that adds concise Markdown, HTML, or image results with tappable links to the Ambient timeline.',
  ...
  'Widgets must be glanceable rather than full reports. Prefer concise Markdown for options, comparisons, plans, schedules, and small tables. For routes, maps, and places, request a useful screenshot, a short caption, and a direct map link. Request HTML only for a genuinely custom visual.',
  'When steering a worker to refine its visible result, tell it to reuse the prior widgetId so the timeline item updates in place. For a distinct result, tell it to omit widgetId and append a new item.',
